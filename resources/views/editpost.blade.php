                    ...
                    <div class="mb-4">
                        ...
                        <select name="color" id="color_id"
                            ...
                            @foreach ($colors as $color)
                                <option {{ $animal->colorId === $color->id ? 'selected':' ' }} value="{{ $color->id }}">{{ $color->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
                        ...
                        <x-input-error :messages="$errors->get('picture')" class="mt-2" />
                            <p class="mt-5 block font-medium text-sm text-gray-700 dark:text-gray-300">Jelenlegi kép (ha nem szeretnél változtatni ne tölts fel semmit!)</p>
                            <img class=" w-[128px] h-[128px] object-contain" src="{{ asset('storage/'.$animal->image) }}" alt="{{ $animal->name }}">
                    </div>
