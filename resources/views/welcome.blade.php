            ...
            <div class="relative sm:flex sm:justify-center sm:items-center min-h-screen bg-center selection:bg-cyan-700 selection:text-white">
                @if (Route::has('login'))
                    <div class="sm:fixed sm:top-0 sm:right-0 p-6 text-right z-10">
                        @auth
                            <a href="{{ url('/dashboard') }}" class="font-semibold text-cyan-600 xl:text-slate-50 hover:text-slate-300 dark:text-gray-100 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-cyan-500">Dashboard</a>
                        @else
                            <a href="{{ route('login') }}" class="font-semibold text-xl text-cyan-600 xl:text-slate-50 hover:text-slate-300 dark:text-gray-100 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-cyan-500">Log in</a>

                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="ml-4 font-semibold text-xl text-cyan-600 xl:text-slate-50  hover:text-slate-300 dark:text-gray-100 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-cyan-500">Register</a>
                            @endif
                        @endauth
                    </div>
                @endif
                ...
                <div class="fixed bottom-0 right-0 p-6 text-right z-1">
                    <p class="font-semibold dark:text-white">website by: Alex Kosik & Mate Cserhati</p>
                </div>
            </div>
